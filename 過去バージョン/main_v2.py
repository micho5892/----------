...
def run_simulation(**kwargs):
    ...
    # ==== 変更：カルマン渦用の形状定義を適用 ====
    build_karman_cylinder(sim.mask, sim.sdf, cfg.nx, cfg.ny, cfg.nz)
    sim.init_fields()
    ...
    k_target_nu = int(cfg.nz * 0.8)  # 円柱の中心断面を指定
    ...
    for step in range(cfg.steps):
        ...
        if step % cfg.vis_interval == 0:
            ...
            top_z = int(cfg.nz * 0.7)             # 平面図 (円柱の直後 XY平面)
            top = temp_np[:, :, top_z]
...
gif_file_val = run_simulation(
    ...
    nz=512,
    Lx_p=0.05,
    U_inlet_p=0.5,  # 代表流速を上昇
    nu_p=3.0e-6,     # 動粘性係数を低下 (Re数を約150付近に設定)
    k_p=0.026,
    steps=6000,      # カルマン渦の発達を見るためステップ数を確保
