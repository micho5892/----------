...
def graetz_parallel_plates_theta_first_mode(eta: np.ndarray) -> np.ndarray:
    """
    ...
    返す配列は、LBM 側と同様に「セル平均が 1」になるよう mean(θ) で正規化する。
    """
    ...
    if not np.any(pos):
        theta = eta * (1.0 - eta)
        return (theta / (np.mean(theta) + 1e-30)).astype(np.float64)
...
def plot_poiseuille_validation(y_coords, w_fluid, t_fluid, U_inlet, out_dir, T_wall=1.0):
    ...
    theta_analytical = graetz_parallel_plates_theta_first_mode(eta)
    denom_t = float(np.mean(t_fluid - T_wall))
    if abs(denom_t) < 1e-12:
        denom_t = -1e-12
    t_normalized = (t_fluid - T_wall) / denom_t
